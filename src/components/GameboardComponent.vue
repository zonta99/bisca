      ...
      <div v-if="gameStore.gamePhase === 'playing'" class="playing-area">
        ...
        <div class="table-cards">
          <div
              ...
              :style="{ transform: getTableCardPosition(index, gameStore.tableCards.length) }"
          >
            <CardComponent :card="card" :show-front="true" />
        ...
        <!-- Current player indicator -->
        <div v-if="gameStore.gamePhase === 'playing'" class="current-player-indicator">
...
const getTableCardPosition = (index, totalCards) => {
  ...
  // Position cards in a slight arc
  const angle = (index - (totalCards - 1) / 2) * 10;
  return `rotate(${angle}deg) translateY(-10px)`;
};
